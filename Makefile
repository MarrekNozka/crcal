...
$(program): $(program).o crstack.o crmem.o
...
#easystack.o: easystack.c easystack.h
...
%.h: %.c
	sed -nr -e '/\/\/%\.h\s*$$/{s@\s*//%\.h@;@;p}' $(patsubst %.h,%.c,$@) >$@.list
	sed -nri -e '1,/@BEGIN@/p' -e '/@BEGIN@/r $@.list' -e '/@END@/,$$p' $@
########################################################################################
